...
public partial class SpaceGraphProto : MonoBehaviour
{
    ...
    public class SpaceGraphGenerator
    {
        ...
        {
            verts.Sort((Vector3 i, Vector3 j) =>
            {
                if (i.x == j.x && i.y == j.y && i.z == j.z)
                    return 0;
                else if (i.x < j.x || (i.x == j.x && i.y < j.y) || (i.x == j.x && i.y == j.y && i.z < j.z))
                {
                    return -1;
                }
                else return 1;
        ...
        public static void RemoveLongEdges(ref IList<Vector3> v, ref IList<SpaceEdge> e, float maxRange, int desirededges = 1)
        {
            float rSqr = maxRange * maxRange;
            List<SpaceEdge>[] myVertToEdgeList = new List<SpaceEdge>[v.Count];
            for (int i = 0; i < v.Count; i++)
                myVertToEdgeList[i] = new List<SpaceEdge>();
            foreach (SpaceEdge edge in e)
            {
                myVertToEdgeList[edge.To].Add(edge);
                myVertToEdgeList[edge.From].Add(edge);
            }
            SpaceEdgeSorter sorter = new SpaceEdgeSorter(v);
            for (int i = 0; i < v.Count; i++)
            {
            ...
            {
                SpaceEdge longestFrom = myVertToEdgeList[e[i].From][0];
                if (myVertToEdgeList[longestFrom.To].Count > desirededges && myVertToEdgeList[longestFrom.From].Count > desirededges)
                {
        ...
        {
            List<SpaceEdge> planetEdges = new List<SpaceEdge>(planetArr.Length);
            for (int i = 0; i < planetArr.Length; i++)
            {
                for (int k = i + 1; k < planetArr.Length; k++)
                {
                    SpaceEdge e = new SpaceEdge() { To = k, From = i };
                    Vector3 midPoint = (planetArr[e.From] + planetArr[e.To]) / 2f;
                    float rSquared = (planetArr[e.From] - midPoint).sqrMagnitude;

                    if (!SphereSquareCheck(midPoint, rSquared, planetArr, i, k))
                    {
                        planetEdges.Add(e);
                    }
                }
        ...
        public static SpaceGraph GenerateSpiral(int seed, SpaceGraphParameters param)// int minPlanets = 15, int maxPlanets = 255, Vector3 scale = default(Vector3), int arms = 1, Vector3 armScale = default(Vector3), Vector3 up = default(Vector3), Vector3 right = default(Vector3), float tangentalVelocity = 1f)
        {
            Vector3[] planetArr;
            using (var randLock = new TempRandomLock(seed))
            {
                int planetsToGen = Random.Range(param.MinNumPlanets, param.MaxNumPlanets + 1);
            ...
            planetArr = SpiralSquared(planetArr, param.Scale.sqrMagnitude + (param.ArmScale / 2f).sqrMagnitude, param.Up, param.TangentalVelocity);
        ...
        //True on collision
        public static bool SphereSquareCheck(Vector3 origin, float radiusSqr, IEnumerable<Vector3> points, params int[] ignores)
        {
