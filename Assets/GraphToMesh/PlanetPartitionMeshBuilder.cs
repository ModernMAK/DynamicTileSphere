...
{
    public PlanetPartitionMeshBuilder(PlanetGraphMeshBuilder parent, PlanetPartition partition) : base()
    {
        Parent = parent;
        Partition = partition;
    }
    public override void Generate()
    {
        if (Partition.IsDirty)
        {
            //Debug.Log("Dirty");
            Clear();
            foreach (PlanetPoly poly in Partition)
                Render(poly);
            Partition.Clean();
        }
    }
    ...
    {
        float t = ((float)(vert * scale + offset)) / (shape * scale) * Mathf.PI * 2f;
    ...
    {
        Vector3 pos = twinNode ? edge.Node.Position : edge.Twin.Node.Position;
    ...
    {
        ProceduralVertex Center = new ProceduralVertex(Vector3.Scale(polygon.CenterLerp, Parameters.Scale(polygon.Elevation))).SetColor(Color.red);
        List<ProceduralVertex> Verts = new List<ProceduralVertex>(6);//Hex has 6, which is the most expected for the dodechasohedron

        Vector3 normal = Center.Position.normalized;
        Vector3 tangent = Quaternion.FromToRotation(Vector3.up, normal) * Vector3.right;
        ...
        bool elevationDiscrepancy = false;
        ...
        for (i = 0; i < l; i++)
        {
            AddTriangle(indicies[l], indicies[(i + 1) % l], indicies[i]);
        }
        ...
        if (Parameters.Solidity < 1f || (Parameters.ElevationOffset != 0f && elevationDiscrepancy))
            foreach (PlanetEdge edge in polygon)
            {
                Render(edge);
            }
    }
    protected override void GenerateEdge(PlanetEdge edge)
    {
        ...
        indicies[0] = AddVertex(new ProceduralVertex(edgeEdgePos).SetColor(Color.red).SetUv3(terrainUv).SetNormal(norm).SetTangent(tan).SetUv(GetUvFromShape(1, 8)));
        indicies[1] = AddVertex(new ProceduralVertex(edgeTwinPos).SetColor(Color.red).SetUv3(terrainUv).SetNormal(norm).SetTangent(tan).SetUv(GetUvFromShape(3, 8)));
        indicies[2] = AddVertex(new ProceduralVertex(twinEdgePos).SetColor(Color.green).SetUv3(terrainUv).SetNormal(norm).SetTangent(tan).SetUv(GetUvFromShape(5, 8)));
        indicies[3] = AddVertex(new ProceduralVertex(twinTwinPos).SetColor(Color.green).SetUv3(terrainUv).SetNormal(norm).SetTangent(tan).SetUv(GetUvFromShape(7, 8)));
    ...
    protected override void GenerateCorner(PlanetEdge edge)
    {
        ...
        Vector3
            terrainUv = new Vector3(edge.Poly.TerrainType, edge.Prev.Twin.Poly.TerrainType, edge.Prev.Twin.Prev.Twin.Poly.TerrainType);
        ...
        //Set norm and tangent
        //Set uv from shape (square is rotated 45 degrees), so we make it 1/8 instead of 0/4 to accomplish this rotation
        indicies[0] = AddVertex(new ProceduralVertex(edgePos).SetColor(Color.red).SetUv3(terrainUv).SetNormal(norm).SetTangent(tan).SetUv(GetUvFromShape(1, 6)));
        indicies[1] = AddVertex(new ProceduralVertex(edgeNextPos).SetColor(Color.green).SetUv3(terrainUv).SetNormal(norm).SetTangent(tan).SetUv(GetUvFromShape(3, 6)));
        indicies[2] = AddVertex(new ProceduralVertex(edgeNextNextPos).SetColor(Color.blue).SetUv3(terrainUv).SetNormal(norm).SetTangent(tan).SetUv(GetUvFromShape(5, 6)));
    ...
    {
        foreach (PolygonEdge<PlanetPoly, PlanetEdge, PlanetNode> edge in Edges)
            RemoveEdge(edge);
        foreach (PolygonCorner<PlanetPoly, PlanetEdge, PlanetNode> corner in Corners)
            RemoveCorner(corner);

        base.Clear();
    }
    public PlanetGraphMeshParameters Parameters { get { return Parent.Parameters; } }
