...
public enum ShapeType { Tetrahedron, Octahedron, Cube, Icosahedron, Dodecahedron };
/// <summary>
/// The radius to use as reference;
/// Normalized will produce shapes based upon a circumradius of 1f
/// Circumradius will produce shapes based upon 
/// </summary>
public enum RadiusType { Circumradius, Midradius, Inradius, Normalized }
public static class ProceduralPlatonicSolidGenerator
{
    ...
    private static ProceduralVertex ModifyVertex(ProceduralVertex proceduralVertex, Quaternion rotation,/* bool normalize = true,*/ bool right = false)
    {
        //if (normalize)
        ...
        Quaternion vertexRotation = Quaternion.FromToRotation(Vector3.forward, proceduralVertex.Normal);
        proceduralVertex.Tangent = vertexRotation * Vector3.right;
        proceduralVertex.RightHanded = right;
        return proceduralVertex;
    }

    private static ProceduralMeshBuilder TetrahedronPrototype = BuildTetrahedronPrototype();
    private static ProceduralMeshBuilder BuildTetrahedronPrototype()
    {
        ProceduralVertex[] verts = TetrahedronVerticies;
        ProceduralMeshBuilder builder = new ProceduralMeshBuilder();
        Quaternion rotation = Quaternion.FromToRotation(Vector3.up, (Vector3)verts[0].Position);
        int vertexCount = verts.Length;

        for (int i = 0; i < vertexCount; i++)
        {
            ProceduralVertex modifiedVertex = ModifyVertex(verts[i], rotation);
    ...
    private static ProceduralMeshBuilder OctahedronPrototype = BuildOctahedronPrototype();
    private static ProceduralMeshBuilder BuildOctahedronPrototype()
    {
        ProceduralVertex[] verts = OctahedronVerticies;
        ProceduralMeshBuilder builder = new ProceduralMeshBuilder();
        Quaternion rotation = Quaternion.FromToRotation(Vector3.up, (Vector3)verts[0].Position);
        int vertexCount = verts.Length;
    ...
    private static ProceduralMeshBuilder CubePrototype = BuildCubePrototype();
    private static ProceduralMeshBuilder BuildCubePrototype()
    {
        ProceduralVertex[] verts = CubeVerticies;
        ProceduralMeshBuilder builder = new ProceduralMeshBuilder();
        ...
        for (int i = 0; i < vertexCount; i++)
        {
            ProceduralVertex modifiedVertex = ModifyVertex(verts[i], rotation, false);
            builder.AddVertex(modifiedVertex);
        }
    ...
    private static ProceduralMeshBuilder IcosahedronPrototype = BuildIcosahedronPrototype();
    private static ProceduralMeshBuilder BuildIcosahedronPrototype()
    {
        ProceduralVertex[] verts = IcosahedronVerticies;
        ProceduralMeshBuilder builder = new ProceduralMeshBuilder();
        Quaternion rotation = Quaternion.FromToRotation(Vector3.up, (Vector3)verts[0].Position);
        int vertexCount = verts.Length;
    ...
    private static ProceduralMeshBuilder DodecahedronPrototype = BuildDodecahedronPrototype();
    private static ProceduralMeshBuilder BuildDodecahedronPrototype()
    {
        ProceduralVertex[] verts = DodecahedronVerticies;
        ProceduralMeshBuilder builder = new ProceduralMeshBuilder();
        Quaternion rotation = Quaternion.FromToRotation(Vector3.up, (Vector3)verts[0].Position); //Quaternion.identity;
        int vertexCount = verts.Length;
        ...
        {
            int l = groups.GetLength(1);
            ProceduralVertex midpoint = new ProceduralVertex();
            for (int j = 0; j < l; j++)
            {
                midpoint += verts[groups[i, j]];
            }
            midpoint /= l;
            midpoint = ModifyVertex(midpoint, rotation);
            int midpointIndex = builder.AddVertex(midpoint);
            for (int j = 0; j < l; j++)
            {
                builder.AddTriangle(midpointIndex, groups[i, j], groups[i, (j + 1) % l]);
            }
        }
    ...
    private static ProceduralMeshBuilder GetBuilderPrototype(ShapeType shape)
    {
        ...
        switch (shape)
        {
            ...
            case ShapeType.Dodecahedron:
                ...
                break;
            default:
                throw new System.Exception("Shape must be of an enumerated type!");
        }
        return builder;
    }
    public static void AddToMeshBuilder(ProceduralMeshBuilder builder, ShapeType shape, RadiusType radius, float scale = 1f)
    {
        ProceduralMeshBuilder prototype = GetBuilderPrototype(shape);//,shape,radius,scale)
        int offset = builder.Verticies.Count;
        foreach (ProceduralVertex v in prototype.Verticies)
            builder.AddVertex(v * GetConversion(shape,radius) * scale);
        foreach (ProceduralTriangle t in prototype.Triangles)
            builder.AddTriangle(t.Pivot + offset, t.Left + offset, t.Right + offset);
    }
    public static ProceduralMeshBuilder CreateBuilder(ShapeType shape, RadiusType radius = RadiusType.Circumradius, float scale = 1f)
    {
        ProceduralMeshBuilder builder = new ProceduralMeshBuilder(GetBuilderPrototype(shape));
        float conversion = GetConversion(shape, radius);
        if (scale * conversion != 1f)
        {
            ProceduralMeshUtility.ModifyRadius(builder, scale * conversion);
        }
    ...
    public static float GetConversion(ShapeType shape, RadiusType radius)
    {
        ...
        throw new System.Exception("Radius must be of an enumerated type!");
    }
}