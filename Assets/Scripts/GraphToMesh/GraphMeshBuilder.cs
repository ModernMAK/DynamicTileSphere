...
namespace GraphToMesh
{
        ...
        where NodeT : PositionNode<PolyT, EdgeT, NodeT>, new()
    {
        ...
        protected virtual void GeneratePolygon(PolyT polygon)
        {
            ...
            for (var i = 1; i < Nodes.Count - 1; i++) AddTriangle(Nodes[0], Nodes[i], Nodes[i + 1]);
        }

        public void Render(EdgeT edge)
        {
            var polyEdge = new PolygonEdge<PolyT, EdgeT, NodeT>(edge);
            var polyCorner = new PolygonCorner<PolyT, EdgeT, NodeT>(edge);
